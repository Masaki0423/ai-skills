...
SKILL.md 形式のスキル集。Claude Code / OpenAI Codex CLI など、SKILL.md 標準に対応した
コーディングエージェントで利用できます。Claude Code 向けにはプラグインマーケットプレイス形式でも配布しています。
...
Codex は SKILL.md 標準に対応しているため、同じスキルがそのまま動きます。
`~/.codex/skills/`（グローバル）またはプロジェクトの `.codex/skills/` にコピーしてください。

```bash
git clone https://github.com/Masaki0423/ai-skills.git
cp -r ai-skills/plugins/ai-community-tools/skills/<skill-name> ~/.codex/skills/
```

スキルは自動検出されます（`/skills` に出てこない場合のみ Codex を再起動）。
環境変数（`SLACK_WEBHOOK_URL` 等）はシェルの環境で設定してください。
...
- **このリポジトリ（github.com）が正**です。ミラー先には直接コミットしないでください
- 変更の提案は fork して PR で。マージ後、メンテナが各ミラーへ同期します
- シークレット（Webhook URL・APIキー等）を含むファイルはコミット禁止です
